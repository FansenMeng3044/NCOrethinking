...
class WarmupBaseline(Baseline):
    ...
    def get_epoch_metrics(self):
        metrics = dict(self.baseline.get_epoch_metrics())
        metrics['warmup_alpha'] = self.alpha
        return metrics
...
class RolloutBaseline(Baseline):
    ...
    def epoch_callback(self, model, epoch):
        ...
        baseline_epoch_before = self.epoch
        baseline_mean_before = self.mean
        difference = candidate_mean - baseline_mean_before
        p_val = None
        updated = False
        print("Epoch {} candidate mean {}, baseline epoch {} mean {}, difference {}".format(
            epoch, candidate_mean, baseline_epoch_before, baseline_mean_before, difference))
        if difference < 0:
            # Calc p value
        ...
        self.last_epoch_metrics = {
            'baseline_candidate_mean': float(candidate_mean),
            'baseline_reference_mean': float(baseline_mean_before),
            'baseline_difference': float(difference),
            'baseline_p_value': None if p_val is None else float(p_val),
            'baseline_updated': updated,
            'baseline_epoch_before': int(baseline_epoch_before),
            'baseline_epoch_after': int(self.epoch),
        }

    def get_epoch_metrics(self):
        return dict(self.last_epoch_metrics)
