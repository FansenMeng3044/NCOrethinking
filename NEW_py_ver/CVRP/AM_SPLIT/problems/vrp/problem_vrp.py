        ...
        if filename is not None:
            ...
            if extension == '.pkl':
                ...
                if depot.dim() == 3 and depot.size(1) == 1:
                    depot = depot[:, 0]
                loc = data['node_xy'].float()
                demand = data['node_demand'].float()
                end = min(offset + num_samples, loc.size(0))
                self.data = [
                    {'depot': depot[i], 'loc': loc[i], 'demand': demand[i]}
                    for i in range(offset, end)
                ]
            else:
                raise ValueError('CVRP fixed datasets must be .pkl or .pt files')
