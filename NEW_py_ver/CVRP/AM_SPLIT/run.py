...
def run(opts):
    ...
    if opts.resume:
        completed_epochs = int(os.path.splitext(os.path.split(opts.resume)[-1])[0].split("-")[1])
        ...
        baseline.epoch_callback(model, completed_epochs - 1)
        print("Resuming after {} completed epochs".format(completed_epochs))
        opts.epoch_start = completed_epochs
    ...
    try:
        if opts.eval_only:
            ...
            metrics_logger = MetricsLogger(opts.save_dir, opts, model, baseline)
        ...
        if metrics_logger is not None:
            metrics_logger.close(status=status, error=error)
