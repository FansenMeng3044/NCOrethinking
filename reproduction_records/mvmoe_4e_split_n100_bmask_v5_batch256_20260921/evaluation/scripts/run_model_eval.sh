...
if [[ "$MODE" == six ]]; then
  ...
  PIDS=()
  launch paired_n200 run_paired 200 8 0
elif [[ "$MODE" == one ]]; then
  if [[ "$TRAINED_SIZE" == 50 ]]; then
    sizes=(50 100 200)
  elif [[ "$TRAINED_SIZE" == 100 ]]; then
    sizes=(100 50 200)
  else
    echo "TRAINED_SIZE must be 50 or 100" >&2
    exit 2
  fi
  for size in "${sizes[@]}"; do
    ...
    PIDS=()
    launch "paired_n${size}" run_paired "$size" "$batch" 0
    wait "${PIDS[0]}"
  done
  PIDS=()
else
  echo "mode must be one, two, three, or six" >&2
  exit 2
fi
