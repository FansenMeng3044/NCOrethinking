"""Evaluate a B-order-masked Split checkpoint on official environments."""
...
def main():
    ...
    if checkpoint.get("decoder_constraints") != ["B"]:
        raise ValueError("checkpoint decoder must enforce only the B ordering constraint")
    ...
    if checkpoint.get("constraint_factorization_version") != 5:
    ...
    with torch.no_grad():
        for problem in problems:
            ...
            print(
                f"{problem}: {len(ok)}/{args.episodes} feasible, mean cost={mean:.6f}",
                flush=True,
            )
