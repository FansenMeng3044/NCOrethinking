"""Run the official POMO-MTL greedy x8 protocol and save per-instance costs."""
...
def build_model(checkpoint_path, device):
    ...
    model = get_model("MTL")(**params).to(device)
    ...
    with torch.no_grad():
        for problem in problems:
            ...
            while offset < args.episodes:
                ...
                offset += count
            current = [r for r in rows if r["problem"] == problem]
            mean = sum(float(r["cost"]) for r in current) / len(current)
