...
run_id=${MVMOE_RUN_ID:-formal_fused_4_1_$(date -u +%Y%m%d_%H%M%S)}
...
if [[ $(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l) -lt 5 ]]; then
    echo "This launcher requires five visible GPUs" >&2
    exit 1
fi
...
start_task mvmoe_4e_split_n100 "0,1,2,3" 4 MOE_SPLIT 100
start_task mvmoe_4el_split_n50 "4" 1 MOE_LIGHT_SPLIT 50
