...
run_id=${MVMOE_RUN_ID:-formal_fused_4el_n100_2gpu_$(date -u +%Y%m%d_%H%M%S)}
run_root="${run_base}/${run_id}"
task=mvmoe_4el_split_n100
...
if [[ $(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l) -lt 2 ]]; then
    echo "This launcher requires two visible GPUs" >&2
    exit 1
fi
...
train_pid=$!
printf '%s\n' "${train_pid}" > "${run_root}/pids/${task}.torchrun.pid"
wait "${train_pid}"
code=$?
