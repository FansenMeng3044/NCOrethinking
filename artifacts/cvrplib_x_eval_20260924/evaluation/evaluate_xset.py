#!/usr/bin/env python3
"""Evaluate AM/POMO direct and Split checkpoints on the official CVRPLIB X set.

Decoding uses greedy AM, one POMO start per customer, and optional eightfold
geometric augmentation. Candidate selection and final verification use the
official per-edge CVRPLIB EUC_2D objective. Reference solutions are read only
after decoding and are never exposed to the models.
"""
...
SCHEMA_VERSION = 2
PROTOCOL = "xset_n100_models_greedy_8fold_official_euc2d_v1"
...
def evaluator_source_hashes(directory: Path) -> dict[str, str]:
    names = ("evaluate_xset.py", "xset_io.py", "summarize_xset.py")
...
def normalized_problem_batch(
    ...
    # X-set coordinates are integer values in [0, 1000]. Matching
    ...
    if np.any(coords < 0) or np.any(coords > 1):
        raise ValueError("X-set coordinates must remain within [0, 1000]")
...
class POMORunner:
    ...
    def decode(self, instances: Sequence[CVRPInstance]) -> CandidateBatch:
        ...
        problem_sizes = {instance.customers for instance in instances}
        if len(problem_sizes) != 1:
            raise ValueError("a POMO batch must contain a single problem size")
        problem_size = problem_sizes.pop()
        ...
        augmentations = np.repeat(np.arange(self.augmentation, dtype=np.int64), problem_size)
        policies = np.tile(np.arange(problem_size, dtype=np.int64), self.augmentation)
...
def flatten_result(
    ...
    return {
        ...
        "nominal_vehicles": features["nominal_vehicles"],
        "status": payload["status"],
        "failure_reason": payload.get("failure_reason", ""),
        "cost": payload.get("cost", ""),
        "reference_cost": payload.get("reference_cost", ""),
        "reference_verified": payload.get("reference_verified", ""),
...
def consolidate_results(
    ...
    for spec in specs:
        ...
        actual_names = {path.stem for path in directory.glob("X-n*-k*.json")}
    ...
    atomic_csv(output / "xset_results.csv", rows)
    return rows
...
def xset_parse_args() -> argparse.Namespace:
    here = Path(__file__).resolve().parent
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--repo", type=Path, required=True)
    parser.add_argument("--models", type=Path, default=here / "models_xset.json")
    parser.add_argument("--data", type=Path, default=here.parent / "data" / "X")
    parser.add_argument("--output", type=Path, required=True)
...
def xset_main() -> None:
    args = xset_parse_args()
    for name in ("repo", "models", "data", "output"):
        setattr(args, name, getattr(args, name).expanduser().resolve())
    if args.limit is not None and args.limit <= 0:
        raise ValueError("--limit must be positive")
    ...
    all_instance_paths = sorted(
        args.data.glob("X-n*-k*.vrp"),
        key=lambda path: (int(re.search(r"-n(\d+)-", path.stem).group(1)), path.stem),
    )
    all_solution_paths = sorted(args.data.glob("X-n*-k*.sol"))
    if len(all_instance_paths) != 100 or len(all_solution_paths) != 100:
        raise ValueError(
            f"expected the complete 100-instance X set, found "
            f"{len(all_instance_paths)} instances and {len(all_solution_paths)} solutions"
        )
    instance_names = {path.stem for path in all_instance_paths}
    if instance_names != {path.stem for path in all_solution_paths}:
        raise ValueError("X-set instance and solution names do not match")
    requested_instances = {name.upper() for name in args.only_instance}
    instance_paths = [
        path
        for path in all_instance_paths
        if not requested_instances or path.stem.upper() in requested_instances
    ]
    if requested_instances - {path.stem.upper() for path in instance_paths}:
        raise ValueError("one or more --only-instance names were not found")
    if args.limit is not None:
    ...
    config = json.loads(args.models.read_text(encoding="utf-8"))
    enabled = [spec for spec in config["models"] if spec.get("enabled", True)]
    ...
    unknown = set(args.only_model) - set(enabled_names)
    if unknown:
        raise ValueError(f"unknown --only-model values: {sorted(unknown)}")
    specs = [spec for spec in enabled if not args.only_model or spec["name"] in args.only_model]
    if not specs:
        raise ValueError("no models selected")
    for spec in specs:
        if spec.get("architecture") not in ARCHITECTURES:
            raise ValueError(f"unsupported architecture: {spec}")
        if int(spec.get("training_size", 0)) != 100:
            raise ValueError(f"{spec['name']}: this run requires the final n=100 checkpoint")
        if args.require_checkpoint_hashes and not spec.get("expected_sha256"):
            raise ValueError(f"{spec['name']}: missing expected_sha256")
    ...
    for spec in specs:
        ...
        started = time.perf_counter()
        runner = build_runner(args.repo, spec, checkpoint, device, args.augmentation)
        preflight_seconds[spec["name"]] = time.perf_counter() - started
        del runner
        if device.type == "cuda":
            torch.cuda.empty_cache()

    selected_names = [path.stem for path in instance_paths]
    dataset_hash_records = [
        f"{path.name}:{sha256_file(path)}:{sha256_file(path.with_suffix('.sol'))}"
        for path in all_instance_paths
    ]
    run_manifest = {
        "schema_version": SCHEMA_VERSION,
        "dataset": "CVRPLIB X",
        "protocol": PROTOCOL,
        "complete_benchmark_requested": len(instance_paths) == 100,
        "instances_requested": len(instance_paths),
        "instance_names_sha256": sha256_lines(selected_names),
        "dataset_identity_sha256": sha256_lines(dataset_hash_records),
        "models_requested": [spec["name"] for spec in specs],
        "models_config_sha256": sha256_file(args.models),
        "checkpoints": metadata_by_model,
        "checkpoint_architecture_preflight_seconds": preflight_seconds,
        "source_bundle_commit": "d1bfa1b535b0ff8d3de2032ab51e1d79bf8566a2",
        "source_repo": git_metadata(args.repo),
        "augmentation": args.augmentation,
        "decode_type": "greedy/argmax",
        "pomo_size": "number of customers in each instance",
        "candidate_selection": "minimum official integer EUC_2D total distance",
        "postprocessing": "none for direct; exact capacity Split for Split architectures",
        "coordinate_normalization": "fixed divisor 1000 for model input only",
        "official_scoring": "nint Euclidean distance per edge, then sum",
        "reference_solution_use": "read only after candidate generation and selection",
        "reference_value_label": "CVRPLIB solution-file reference cost",
        "seed": args.seed,
        "device": str(device),
        "python": sys.version,
        "platform": platform.platform(),
    ...
    if args.resume and manifest_path.exists():
        ...
        for key in immutable:
            if previous.get(key) != run_manifest.get(key):
                raise ValueError(f"resume manifest mismatch for {key}")
        previous_hashes = {
            name: item["sha256"] for name, item in previous["checkpoints"].items()
        }
        current_hashes = {
            name: item["sha256"] for name, item in run_manifest["checkpoints"].items()
        }
        if previous_hashes != current_hashes:
            raise ValueError("resume checkpoint hashes do not match")
        run_manifest["started_at_utc"] = previous["started_at_utc"]
    ...
    for spec in specs:
        ...
        for path in instance_paths:
            ...
            if existing.exists():
                validate_existing_result(
                    existing, spec, path.stem, checkpoint_meta["sha256"]
                )
            else:
                pending.append(path)
        if not pending:
        ...
        runner = build_runner(
            args.repo, spec, checkpoint_by_model[model_name], device, args.augmentation
        )
        print(f"{model_name}: loaded; pending={len(pending)}", flush=True)
        for index, path in enumerate(pending, start=1):
            instance = parse_instance(path)
            candidates = runner.decode([instance])
            sequences = candidates.sequences[0]
            post_started = time.perf_counter()
            split_tours = split_costs = split_predecessor = None
            if candidates.kind == "depot_actions":
                selection = select_depot_actions(
                    instance,
                    sequences,
                    candidates.augmentation_indices,
                    candidates.policy_indices,
                )
            else:
                split_tours, split_costs, split_predecessor = split_giant_tour_dp(
                    instance, sequences
            ...
            if args.augmentation == 1:
                no_aug_selection = selection
            elif candidates.kind == "depot_actions":
                no_aug_selection = select_depot_actions(
                    instance,
                    sequences,
                    candidates.augmentation_indices,
            ...
            reference = parse_solution(path.with_suffix(".sol"))
            ...
            if not reference_check.feasible or reference_check.cost != reference.declared_cost:
                raise ValueError(f"{instance.name}: reference solution failed verification")
            final_check = verify_routes(instance, selection.routes)
            if not final_check.feasible or final_check.cost != selection.cost:
                raise AssertionError(f"{model_name}/{instance.name}: final verification failed")
            ...
            payload = {
                ...
                "absolute_error": selection.cost - reference_cost,
                "gap_percent": gap_percent(selection.cost, reference_cost),
                "vehicles": selection.vehicles,
                "routes": selection.routes,
                "no_aug_status": "ok",
                "no_aug_failure_reason": None,
                "no_aug_cost": no_aug_selection.cost,
                "no_aug_absolute_error": no_aug_selection.cost - reference_cost,
                "no_aug_gap_percent": gap_percent(no_aug_selection.cost, reference_cost),
                "no_aug_vehicles": no_aug_selection.vehicles,
                "no_aug_routes": no_aug_selection.routes,
                "no_aug_candidate_index": no_aug_selection.candidate_index,
                "no_aug_policy_index": no_aug_selection.policy_index,
                "no_aug_candidates_generated": no_aug_selection.candidates_generated,
                "candidate_index": selection.candidate_index,
                "augmentation_index": selection.augmentation_index,
                "policy_index": selection.policy_index,
                "candidates_generated": selection.candidates_generated,
                "candidates_checked_until_selection": selection.candidates_checked_until_selection,
                "candidate_feasibility_scope": "selected candidate independently verified",
                "inference_seconds_amortized": candidates.inference_seconds_amortized,
                "postprocess_seconds": time.perf_counter() - post_started,
                "physical_batch_size": candidates.physical_batch_size,
                "checkpoint": checkpoint_meta,
                "created_at_utc": utc_now(),
            }
            atomic_json(result_path(args.output, model_name, instance.name), payload)
            print(
                f"{model_name}: {index}/{len(pending)} {instance.name} "
                f"cost={selection.cost} gap={payload['gap_percent']:.3f}%",
                flush=True,
            )
        del runner
        if device.type == "cuda":
    ...
    rows = consolidate_results(args.output, specs, selected_names)
    expected_rows = len(specs) * len(instance_paths)
    if len(rows) != expected_rows:
        raise ValueError(f"expected {expected_rows} rows, found {len(rows)}")
    run_manifest["completed_at_utc"] = utc_now()
    run_manifest["wall_seconds_this_invocation"] = time.perf_counter() - wall_started
    run_manifest["model_invocation_timings"] = model_timings
    run_manifest["result_rows"] = len(rows)
    run_manifest["status"] = "complete" if len(instance_paths) == 100 else "partial"
    atomic_json(manifest_path, run_manifest)
    print(f"saved {len(rows)} rows to {args.output / 'xset_results.csv'}", flush=True)
