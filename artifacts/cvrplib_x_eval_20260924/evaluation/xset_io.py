#!/usr/bin/env python3
"""Strict CVRPLIB X-set parsing, scoring, Split, and verification.
...
XSET_RE = re.compile(r"^X-n(\d+)-k(\d+)$")
...
class XSetFeatures:
    ...
    def as_dict(self) -> dict[str, int]:
        return {
            ...
            "nominal_vehicles": self.nominal_vehicles,
        }
...
def parse_xset_name(name: str) -> XSetFeatures:
    match = XSET_RE.fullmatch(name)
    if match is None:
        raise ValueError(f"not a CVRPLIB X-set name: {name}")
    return XSetFeatures(*(int(value) for value in match.groups()))
...
def parse_instance(path: str | Path) -> CVRPInstance:
    ...
    if headers["EDGE_WEIGHT_TYPE"].upper() != "EUC_2D":
        raise ValueError(f"{path}: X set requires EDGE_WEIGHT_TYPE=EUC_2D")
    ...
    if set(coords_by_id) != set(range(1, dimension + 1)):
        raise ValueError(f"{path}: node IDs must be consecutive 1..{dimension}")
    if len(depot_ids) != 1:
        raise ValueError(f"{path}: expected exactly one depot, got {depot_ids}")
    depot_id = depot_ids[0]
    if depot_id != 1:
        raise ValueError(f"{path}: X-set depot node ID must be 1, got {depot_id}")
    ...
    if (coords < 0).any() or (coords > 1000).any():
        raise ValueError(f"{path}: X-set coordinates must lie in [0, 1000]")
    name = headers["NAME"]
    if path.stem != name:
        raise ValueError(f"{path}: filename/name mismatch ({path.stem!r} != {name!r})")
    features = parse_xset_name(name)
    if features.dimension != dimension:
        raise ValueError(
            f"{path}: dimension encoded in name is {features.dimension}, header gives {dimension}"
        )
